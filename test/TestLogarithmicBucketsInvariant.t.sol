...
contract LogarithmicBucketsSeenMock is LogarithmicBucketsMock {
    ...
    bytes32[] public seen;
    mapping(bytes32 => bool) public isSeen;

    function seenLength() public view returns (uint256) {
        return seen.length;
    }

    function update(bytes32 _id, uint256 _newValue) public override {
        if (!isSeen[_id]) {
            isSeen[_id] = true;
            seen.push(_id);
        }
    ...
    function getPrev(uint256 _value, bytes32 _id) public view returns (bytes32) {
        BucketDLL.List storage bucket = bucketList.getBucketOf(_value);
        return bucket.getPrev(_id);
    }

    function getNext(uint256 _value, bytes32 _id) public view returns (bytes32) {
        BucketDLL.List storage bucket = bucketList.getBucketOf(_value);
...
contract TestLogarithmicBucketsSeenInvariant is Test, Random {
    ...
    function invariantNotZeroAccountIsInserted() public {
        for (uint256 i; i < buckets.seenLength(); i++) {
            bytes32 user = buckets.seen(i);
            uint256 value = buckets.getValueOf(user);
            if (value != 0) {
                bytes32 next = buckets.getNext(value, user);
                bytes32 prev = buckets.getPrev(value, user);
